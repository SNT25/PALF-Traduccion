init python:
    def DoMove(action, user, move, targets, alreadypretext="", alreadyposttext="", repeating=False, parentalbond=False):
        ...
        if (not MoveValid(user, move)):
            action.ChangeSuccess(False)
            renpy.say(None, "¡{} no puede usar {}!".format(username, move.TranslatedName))
            return
        ...
        if (not repeat):
            if (user.HasStatus("flinching")):
                posttext += "¡{} ha retrocedido!".format(username)
                abortmove = True
                ...
                if (random.random() > .2 
                    ...
                    pretext += "¡{} esta congelado!".format(username)
                    abortmove = True
                else:
                    pretext += "¡{} se ha descongelado!".format(username)
                    user.ClearStatus("frozen")
            elif (user.GetStatusCount("asleep") > 0 and name not in ["Snore", "Sleep Talk"]):
                pretext = "{} está profundamente dormido...".format(username)
                abortmove = True
            elif (user.HasStatus("asleep") and user.GetStatusCount("asleep") <= 0):
                user.ClearStatus("asleep")
                pretext = "¡{} se desperto!".format(username, move.TranslatedName)
            elif (user.HasStatus("paralyzed") and random.random() <= .25):
                posttext += "{} está paralizado y no puede moverse.".format(username)
                abortmove = True
            elif (user.HasStatus("infatuated") and random.random() <= .5):
                posttext += "{} se ha enamorado y no puede moverse.".format(username)
                abortmove = True
            elif (user.HasStatus("confused") and user.GetStatusCount("confused") <= 0):
                user.ClearStatus("confused")
                pretext = "¡{} salió de la confusión!".format(username, move.TranslatedName)
            elif (user.GetStatusCount("confused") > 0):
                pretext = "¡{} esta confundido! ".format(username)
                if (random.random() <= .33):
                        posttext += "¡{} esta tan confundido que se hirió a sí mismo!".format(username)
                        if (not user.HasStatus("busted disguise") and user.HasAbility("Disguise")):
                            ...
                            posttext += "¡El disfraz de {} se ha roto!".format(username)
                        else:
                            DoDamage(user, hurtself, user)
                        abortmove = True
            elif (user.HasStatus("taunted") and move.Category == "Status"):
                posttext += "¡{} esta demasiado enojado para usar {}!".format(username, move.TranslatedName)
                abortmove = True
            elif (user.HasStatus("encored") and move != GetLastMove(ActionLog, user)):
                posttext += "¡{} ha sufrido los efectos de Otra Vez!".format(username)
                abortmove = True
            elif (IsExplosion(name) and AbilityOnField("Damp")):
                posttext += "¡Esta demasiado humedo para hacer una explosión!"
                abortmove = True
        ...
        if (move.Name in moveboosts):
            pretext += "¡Los deseos de {} se han unido! ".format(username)

        pretext += "¡{} ha usado {}!".format(username, move.TranslatedName)
        ...
        if (StatusOnFoes(user, "snatching") != None and IsSnatchable(name)):
            ...
            targets = [user]
            pretext += "¡El movimiento fue robado por {}!".format(username)
        ...
        for mon in targets:
            if (mon != None):
                if (user not in GetBattlers(mon)
                    ...
                    pretext += "¡Ha seleccionado a {}, pero el movimiento ha rebotado de vuelta!".format(mon.GetNickname())
                    bouncingmon = mon
                    break
        ...
        for i, target in enumerate(targets):
            ...
            if (name in ["Thrash", "Outrage", "Uproar", "Petal Dance", "Raging Fury"]):
                ...
                if (target not in GetBattlers(user, True)):
                    ...
                    if (target == None):
                        renpy.say(None, "¡{} ha usado Venganza! \n¡Pero ha fallado!".format(username, name))
                        UsingMove = False
                        MoveUser = None
                        ActiveMove = None
                        action.ChangeSuccess(False)
            ...
            if ((target == None or target != None and target.GetHealthPercentage() <= 0) and len(targets) < 2):
                action.ChangeSuccess(False)
                renpy.say(None, "¡{} intento usar {}, pero no hay ningun objetivo!".format(username, move.TranslatedName))
                UsingMove = False
                MoveUser = None
                ActiveMove = None
                return
            ...
            targetstring = " ¡Ha seleccionado a " + target.GetNickname() + "!"
            ...
            and name not in ["Tearful Look", "Hyper Drill", "Bestow", "Toxic Spikes", "Spikes", "Stealth Rock"]
            and not (name in ["Fly", "Bounce"] and not user.HasStatus("airborne")
                ...
                repeat = False                  
                doDamage = False
                posttext += "¡Pero {} estaba protegido!".format(target.GetNickname())
                if (name in ["Jump Kick", "High Jump Kick"]):
                    user.AdjustHealth(-user.GetStat(Stats.Health) / 2.0)
                    posttext += "¡{} falló y se hizo daño por la caída!".format(username)
                ...
                doDamage = False
                posttext += "¡Pero falló contra {}!".format(target.GetNickname())
                repeat = False
                MultihitMax = None
                MultihitCount = None
                if (name in ["Jump Kick", "High Jump Kick"]):
                ...
                doDamage = False
                pretext += "No afectó a {}...".format(target.GetNickname())
            ...
            #move effects under here
            ...
            elif (name in ["Crunch", "Liquidation"]):
                ...
                if (MultihitCount != 0):
                    ...
                    MultihitMax = None
                    MultihitCount = None
            elif (name in ["Double Kick", "Bonemerang", "Dual Chop", "Double Hit", "Twineedle", "Dual Wingbeat"]):
                global MultihitCount
                global MultihitMax
                repeat = True
                if (name == "Twineedle"):
                ...
                user.ApplyStatus("transformed", target)
                ...
                posttext += ApplyBattlefieldEffects("Magic Room", 5)
            elif (name == "Soak"):
                posttext += target.ApplyStatus("soaked")
            elif (name == "Wonder Room"):
                posttext += ApplyBattlefieldEffects("Wonder Room", 5)
                ...
                if (CalculateFling(user) != 0) and not (user.HasAbility("Klutz") or user.HasStatus("embargoed") or BattlefieldExists("Magic Room")):
                    ...
                    if item in [Item.KingsRock, Item.RazorFang]:
                        sideeffects.append(SideEffect(user, target, "flinching"))
                    elif item == Item.PoisonBarb:
                        posttext += target.ApplyStatus("poisoned", applier = user)
                else:
                    doDamage = False
                    action.ChangeSuccess(False)
            ...
            if (doDamage and typebonus != 0):
                ...
                if (doDamage):
                    ...
                    RunItemFunction("takingDirectDamage", user, [])
                    ...
                    if (typebonus > 1 and fixeddamage == -1):
                        ...
                        pretext += "It's not very effective on {}...\n".format(target.GetNickname())
                    ...
                    if (contact and random.random() <= 0.3 and not (user.HasType("Grass") or RunItemFunction("blockEffect", user, ["spore"])) and target.HasAbility("Effect Spore") and not user.HasAbility("Overcoat")):
                        ...
                        if (newrand <= 0.3):
                            user.ApplyStatus("poisoned", 1, target)
                        elif (newrand <= .6333):
                    ...
                    if (move.Category != "Status" and target.HasAbility("Cursed Body") and random.random() <= 0.3):
            ...
            if (lookupforeveraldata(fvl, FVLMacros.FVLType) == ForeveralTypes.Training):
                if (fvl == "Tyrogue Everal"):
                    ...
                    if (move.Category == "Physical"):
                        ...
                        user.EVs[0] = min(user.EVs[0] + 3, 255)
