init python:
    ...
    def AbilityOnOpponentField(mon, abilityname, splash=True):
        if (mon.GetTrainerType() != TrainerType.Enemy):
        ...
        for trainer in Trainers:
            for mon in trainer.GetTeam():
                ...
                if (IsAfter(10, 5, 2004) or (IsDate(9, 5, 2004) and timeOfDay not in ["Noon", "Morning", "Afternoon"])):
                    mon.Trained = []
                    for move in ["Chrysalize", "Enshroud", "Legacy", "Energize", "Ardent Gaze", "Disabling Poke", "Steady Flame", "Wing It", "Deathless", "Bark Up", "Burial Ground", "Slow Freeze", "Simple World", "Bad Breath", "Clear Mind", "Splinter Shield", "Metallize", "Healing Spring", "Stone Cold Stunner"]:
    ...
    def ReactivateAbility(mon, fromTrace = False):
        ...
        for ability in mon.GetAbilities():
            ...
            if (mon.HasAbility("Imposter")):
                ...
                mon.ApplyStatus("transformed", target)
